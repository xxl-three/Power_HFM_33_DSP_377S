...
void InitGpio()
{
    ...
    GpioDataRegs.GPADAT.all = 0x40000000;       // GPIO Data GPIO00-GPIO31
    GpioDataRegs.GPBDAT.all = 0x00800000;       // GPIO Data GPIO32-GPIO63
    GpioDataRegs.GPCDAT.all = 0x00040000;       // GPIO Data GPIO64-GPIO95
    GpioDataRegs.GPDDAT.all = 0x00000000;
    GpioDataRegs.GPEDAT.all = 0x00000000;

    // GPIO Direction
    GpioCtrlRegs.GPADIR.all = 0xDFFFFFFF;       //0xECFFFFFF;   //31~0
    GpioCtrlRegs.GPBDIR.all = 0xA38FB763;       //0xBB5D64E3;   //63~32
    GpioCtrlRegs.GPCDIR.all = 0x8FF00B00;       //0xFD799FFE;   //95~64
	...
	//GpioCtrlRegs.GPBQSEL2.bit.GPIO61 = 2;        // XINT4 Qual using 6 samples
	//GpioCtrlRegs.GPCQSEL1.bit.GPIO64 = 2;        // XINT5 Qual using 6 samples
	//GpioCtrlRegs.GPCQSEL1.bit.GPIO65 = 2;        // XINT3 Qual using 6 samples
	
	EDIS;
	
	//GPIO_SetupXINT3Gpio(65);
	//GPIO_SetupXINT4Gpio(61);
	//GPIO_SetupXINT5Gpio(64);
...
void GPIO_SetupPinMux(Uint16 pin, Uint16 cpu, Uint16 peripheral)
{
    ...
    pin16 = pin % 16;
    pin8 = pin % 8;
    gpioBaseAddr = (Uint32 *)&GpioCtrlRegs + (pin/32)*GPY_CTRL_OFFSET;//GPY_CTRL_OFFSET=0X40/2,每段数据为16位二进制数

    //
    //Sanity check for valid cpu and peripheral values
    //
    if (cpu > GPIO_MUX_CPU2CLA || peripheral > 0xF)//3
    {
        return;
    }
    ...
    //
    mux = gpioBaseAddr + GPYMUX + pin32/16;                           //GPYMUX=3第三位往后16位
    gmux = gpioBaseAddr + GPYGMUX + pin32/16;                         //GPYGMUX=10
    csel = gpioBaseAddr + GPYCSEL + pin32/8;                          //GPYCSEL=14
...
{
    volatile Uint32 *gpioBaseAddr;
    volatile Uint32 *dir, *pud, *inv, *odr, *qsel;        //odr： Open Drain Output Register
