...
//   
//----------------------------------------------------------------------------
//#define SET_INTERRUPT_PRIORITY
#pragma CODE_SECTION(ADCA1_ISR, "ramfuncs");
interrupt void ADCA1_ISR(void)                    // 50us
{
#ifdef SET_INTERRUPT_PRIORITY
    ...
    //IsrAdcSampling();                         //760ns/200Mhz
    //IsrAdcSum();                              //1.06¦Ìs/200Mhz
    

    //IsrBuzzerPwm();
    
//#ifdef DFT_PLL_ENABLE
//    DftIncrementSysThetaIsr();
//    DftFilterIsr();                         //780ns/200Mhz
//#endif
...
interrupt void TIMER0_ISR(void)
{
    ...
    // Insert ISR Code here
    //
    u32StartIsrTimeCnt = ECap2Regs.TSCTR;
    ...
    AdcdRegs.ADCSOCFRC1.all = 0x00FF;
       
    IsrInverterController();
    
    SystemControl_ISR();    
    
    //IsrWaveRecord();

    u32EndIsrTimeCnt = ECap2Regs.TSCTR;
    u32CpuIsrTime = labs(u32EndIsrTimeCnt - u32StartIsrTimeCnt)*5;
    if(u32CpuIsrTime > u32IsrTimeMax)
    {
        u32IsrTimeMax = u32CpuIsrTime;
    }
    //
    // To receive more interrupts from this PIE group,
    // acknowledge this interrupt.
