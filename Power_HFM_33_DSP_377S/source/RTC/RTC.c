...
// 具体来说，通过位运算和逻辑运算，从每个寄存器中的数据中提取出对应的时间信息。
// 例如，对于秒钟寄存器，先将高4位右移4位并与0x0007进行按位与运算，得到秒钟的十位数；
...
//让我们以秒钟寄存器为例来解释位运算的工作原理：sCurrent_Time.bSec = ((u16RTCData[RTC_SECOND] >> 4) & 0x0007) * 10 + (u16RTCData[RTC_SECOND] & 0x000F);
